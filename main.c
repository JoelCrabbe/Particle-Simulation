...
int check_position(Particle* particle){
    ...
    if(particle->position.y - particle->radius <= -22.5f || particle->position.y + particle->radius >= 22.50f){
...
int check_collisions(Particle* p1, Particle* p2){
    ...
    if(distance <= p1->radius + p2->radius){
        //if the particles have collided we need to nudge them a little bit
...
int main(void){
    ...
    Particle p1 = {{10.0f,15.0f,0.0f}, {4.0f,1.0f,0.0f}, {4.0f,-4.905f,0.0f}, 0.2f}; 
    Particle p2 = {{-12.0f,-15.0f,0.0f}, {8.0f,-4.0f,0.0f}, {3.0f,-4.905f,0.0f}, 0.2f};
    Particle p3 = {{4.0f,0.0f,0.0f}, {1.0f,-4.0f,0.0f}, {-2.0f,-4.905f,0.0f}, 0.2f};
    ...
    */
    ...
    while(!WindowShouldClose()){
        ...
        Vector2 right_edge = GetWorldToScreen(p1.position, camera);

        //printf("(%f, %f)\n", right_edge.y, p1.position.y);

        //printf("(%f, %f)\n", right_edge.x, p1.position.x);
